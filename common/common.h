...
private:
  ...
  bool write_header(const uint32_t sample_rate,
    ...
    fstream.write("\0\0\0\0", 4);    // Placeholder for file size
    ...
    fstream.write(reinterpret_cast<const char *>(&sub_chunk_size), 4);
    fstream.write(reinterpret_cast<const char *>(&audio_format), 2);
    fstream.write(reinterpret_cast<const char *>(&channels), 2);
    fstream.write(reinterpret_cast<const char *>(&sample_rate), 4);
    fstream.write(reinterpret_cast<const char *>(&byte_rate), 4);
    fstream.write(reinterpret_cast<const char *>(&block_align), 2);
    fstream.write(reinterpret_cast<const char *>(&bits_per_sample), 2);
    fstream.write("data", 4);
    fstream.write("\0\0\0\0", 4);    // Placeholder for data size
  ...
  bool write_audio(const int16_t *data, size_t length) {
    ...
    if (fstream.is_open()) {
      ...
      fstream.write(reinterpret_cast<char *>(&fileSize), 4);
      fstream.seekp(40, std::ios::beg);
      fstream.write(reinterpret_cast<char *>(&dataSize), 4);
